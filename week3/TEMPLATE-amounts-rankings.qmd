...
sui_severity <- ca_region %>%
  ...
  mutate(month = month.abb[month], # order months Oct > Sep (water year)
         month = factor(month, 
...
#| fig-asp: 0.9
#.........................create heatmap.........................
ggplot(heatmap_order, aes(x = year, 
                          y = subregion_name, 
                          fill = annual_mean_sui)) +
  geom_tile() +
...
sbc_subbasin_monthly <- ca_region %>% 
  mutate(subbasin_HUC = str_sub(string = huc12_id, 
                                start = 1, end = 8)) %>% 
  ...
  mutate(month = month.abb[month],
          month = factor(month, 
                         levels = rev(c(month.abb[10:12], month.abb[1:9]))))
```
