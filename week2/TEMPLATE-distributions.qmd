...
mko_clean <- mko %>%
  ...
  date_time = make_datetime(year, month, day, tz = "America/Los_Angeles") +
    ...
    # decimal_time is fraction of day; 86400 seconds in a day
    seconds(decimal_time * 86400)) %>%
...
# scale down hist to match density curve
ggplot(mko_clean, aes(x = Temp_bot, y = after_stat(density))) + 
...
mko_clean %>% 
  ...
  # `rel_min_height` sets threshold for relative height of density curves 
  # (any values below threshold treated as 0); 
  # `scale` controls extent to which different densities overlap
...
mko_clean %>%  
  ...
  # `after_stat(x)` tells ggplot: "Don't use the original data for coloring. 
  # Instead, wait until you've drawn the density curve, 
  # then color each part of that curve based on its x-axis position (Temp_bot)." 
  # Creates a color gradient that flows along the curve from cold to hot temp
  ggridges::geom_density_ridges_gradient(rel_min_height = 0.01, scale = 3) +
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", 
                                  "#D9E7EC", "#EF8080", "#8B3A3A"))
...
ggplot(mko_clean, aes(x = month_name, y = Temp_bot, fill = month_name)) +
  ...
  # Alt way to reorder groups within ggplot, rather than during data wrangling
